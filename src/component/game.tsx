...
import {getGameStatus, move} from '../lib/data';
...
function Page1({onGameFinish}: ParentProps) {
  ...
  const initGame = async () => {
    ...
    setPlayerOneName(gameStatus.playerOneName);
    setPlayerTwoName(gameStatus.playerTwoName);
    setLoading(false);
  ...
  const getWinnerComponent = () => {
    ...
    return (
      <div className='shakespeare'>
        <p>{text}</p>
        <Link className='fancy-link' to="/">Home</Link>
      </div>
    );
  };
  ...
  const getPlayerComponent = (compPlayer:number)  => {
    let titleText;
    if (compPlayer == 0)
      titleText = playerOneName;
    else
      titleText = playerTwoName;
